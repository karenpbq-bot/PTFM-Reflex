...
def conectar():
    ...
    if not url or not key:
        raise ValueError(
            "Error: No se encontraron las credenciales SUPABASE_URL o SUPABASE_KEY en la configuración."
        )
...
def obtener_avance_por_hitos(id_proyecto, df_productos_filtrados=None):
    ...
    ids = df_p["id"].tolist()
    res_seg = (
        supabase.table("seguimiento").select("hito").in_("producto_id", ids).execute()
    )
...
def obtener_gantt_real_data(id_p):
    ...
    try:
        supabase = conectar()
        res_prods = (
            supabase.table("productos").select("id").eq("proyecto_id", id_p).execute()
        )
