...
def mostrar(supervisor_id=None):
    ...
    st.markdown(
        f"### Seguimiento: <span style='color:#FF8C00;'>{nombre_p_act}</span>",
        unsafe_allow_html=True,
    )
    with st.expander(
        "🔍 Selección de Proyecto", expanded=not st.session_state.get("id_p_sel")
    ):
        c1, c2 = st.columns([2, 1])
        bus_p = c1.text_input(
            "Filtrar proyecto por nombre o código:", key="bus_seg_VMASTER"
        )
        ...
        if not df_p_all.empty:
            opciones = {
                f"[{r['codigo']}] {r['proyecto_text']}": r["id"]
                for _, r in df_p_all.iterrows()
            }
            ...
            if sel_n != p_actual:
                st.session_state.id_p_sel = (
                    opciones[sel_n] if sel_n != "-- Seleccionar --" else None
                )
                st.session_state.p_nom_sel = sel_n
                st.session_state.cambios_pendientes = []
                st.rerun()
        else:
            st.warning("No se encontraron proyectos asignados.")
            return
    if not st.session_state.get("id_p_sel"):
        st.info("💡 Seleccione un proyecto.")
        return
    ...
    res_db = (
        ...
        .select("*")
        .in_("producto_id", prods_all["id"].tolist())
        .execute()
    )
    segs = (
        pd.DataFrame(res_db.data)
        if res_db.data
        else pd.DataFrame(columns=["producto_id", "hito", "fecha", "observaciones"])
    )
    pesos = obtener_pesos_seguimiento()
    with st.expander("⚙️ Herramientas y Gestión Masiva"):
        t1, t2, t3, t4 = st.tabs(
            ["⚖️ Ponderación", "🔍 Filtros Avanzados", "📥 Importar", "📤 Exportación"]
        )
        with t1:
            cols_w = st.columns(4)
            for i, h in enumerate(HITOS_LIST):
                pesos[h] = cols_w[i % 4].number_input(
                    f"{h} (%)", value=float(pesos.get(h, 12.5)), key=f"pw_{h}"
                )
        with t2:
            f1, f2, f3 = st.columns(3)
            agrupar_por = f1.selectbox(
                "Agrupar por:", ["Sin grupo", "Ubicación", "Tipo"], key="f_agrup"
            )
        ...
        with t3:
            ...
            f_av = st.file_uploader(
                "Subir archivo (.xlsx)", type=["xlsx"], key="up_excel_seg"
            )
            if f_av and st.button("🚀 Iniciar Importación"):
                try:
                    ...
                    for _, r_ex in df_imp.iterrows():
                        u, t = (
                            str(r_ex.get("ubicacion", "")).strip(),
                            str(r_ex.get("tipo", "")).strip(),
                        )
                        match = prods_all[
                            (prods_all["ubicacion"].astype(str).str.strip() == u)
                            & (prods_all["tipo"].astype(str).str.strip() == t)
                        ]
                        if not match.empty:
                            ...
                            for h_nom in HITOS_LIST:
                                if (
                                    h_nom in r_ex
                                    and pd.notnull(r_ex[h_nom])
                                    and (
                                        str(r_ex[h_nom]).strip().upper()
                                        in ["X", "1", "SI"]
                                    )
                                ):
                                    lote_imp.append(
                    ...
                    if lote_imp:
                        supabase.table("seguimiento").upsert(
                            lote_imp, on_conflict="producto_id, hito"
                        ).execute()
        ...
        with t4:
            ...
            for h in HITOS_LIST:
                df_exp[h] = df_exp["id"].apply(
                    lambda x: segs[(segs["producto_id"] == x) & (segs["hito"] == h)][
                        "fecha"
                    ].iloc[0]
                    if not segs[(segs["producto_id"] == x) & (segs["hito"] == h)].empty
                    else ""
                )
            output = io.BytesIO()
            ...
            st.download_button(
                ...
                file_name=f"Reporte_Avance_{id_p}.xlsx",
                use_container_width=True,
            )
    df_f = prods_all.copy()
    if bus_u:
        df_f = df_f[df_f["ubicacion"].astype(str).str.contains(bus_u, case=False)]
    if bus_t:
        df_f = df_f[df_f["tipo"].astype(str).str.contains(bus_t, case=False)]
    st.divider()
    ...
    btn_db = col_b2.button(
        "🚀 Guardar Avances", type="primary", use_container_width=True
    )
    btn_clean = col_b3.button("🧹 Limpiar Marcación", use_container_width=True)
    btn_delete = (
        col_b4.button("🗑️ Borrar Avances", use_container_width=True) if es_jefe else None
    )
    df_editor = df_f[["id", "codigo_etiqueta", "ubicacion", "tipo", "ml", "ctd"]].copy()
    for h_nom in HITOS_LIST:
        simb = MAPEO_HITOS[h_nom]
        en_db = df_editor["id"].apply(
            lambda x: True
            if not segs[(segs["producto_id"] == x) & (segs["hito"] == h_nom)].empty
            else False
        )
        en_mem = df_editor["id"].apply(
            lambda x: True
            if any(
                (
                    c["pid"] == x and c["hito"] == h_nom
    ...
    df_editor["Observaciones"] = df_editor["id"].apply(
        lambda x: segs[(segs["producto_id"] == x) & (segs["hito"] == HITOS_LIST[0])][
            "observaciones"
        ].iloc[0]
        if not segs[(segs["producto_id"] == x) & (segs["hito"] == HITOS_LIST[0])].empty
        else ""
    )
    cambios_df = st.data_editor(
        ...
        column_config={
            ...
            **{
                MAPEO_HITOS[h]: st.column_config.CheckboxColumn(MAPEO_HITOS[h])
                for h in HITOS_LIST
            },
        },
        hide_index=True,
    ...
    if btn_marcar:
        ...
        for idx, row in cambios_df.iterrows():
            ...
            for h_nom in HITOS_LIST:
                simb = MAPEO_HITOS[h_nom]
                v_db = not segs[
                    (segs["producto_id"] == pid) & (segs["hito"] == h_nom)
                ].empty
    ...
    if btn_db:
        ...
        for p in st.session_state.cambios_pendientes:
            match_db = segs[
                (segs["producto_id"] == p["pid"]) & (segs["hito"] == p["hito"])
            ]
            obs_actual = (
                str(match_db["observaciones"].iloc[0]) if not match_db.empty else ""
            )
            if p["hito"] == HITOS_LIST[0]:
                obs_actual = str(
                    cambios_df[cambios_df["id"] == p["pid"]]["Observaciones"].iloc[0]
                )
            lote.append(
                {
                    "producto_id": p["pid"],
        ...
        if lote:
            try:
                ...
                sincronizar_avances_estructural(
                    st.session_state.p_nom_sel.split("]")[0][1:]
                )
                st.session_state.cambios_pendientes = []
                st.cache_data.clear()
    ...
    if btn_delete and es_jefe:
        for idx, row in cambios_df.iterrows():
            pid = int(row["id"])
            for h_nom in HITOS_LIST:
                if not row[MAPEO_HITOS[h_nom]] and (
                    not segs[
                        (segs["producto_id"] == pid) & (segs["hito"] == h_nom)
                    ].empty
                ):
    ...
    def calc_v(df_m, df_s, pend):
        if df_m.empty:
            return 0.0
        ids = df_m["id"].tolist()
        db_v = df_s[df_s["producto_id"].isin(ids)].drop_duplicates(
            subset=["producto_id", "hito"]
        )
        ...
        for p in pend:
            if (
                p["pid"] in ids
                and df_s[
                    (df_s["producto_id"] == p["pid"]) & (df_s["hito"] == p["hito"])
                ].empty
            ):
    ...
    m1, m2 = st.columns(2)
    m1.metric(
        "Avance Parcial", f"{calc_v(df_f, segs, st.session_state.cambios_pendientes)}%"
    )
    m2.metric(
        "Avance Global",
        f"{calc_v(prods_all, segs, st.session_state.cambios_pendientes)}%",
    )