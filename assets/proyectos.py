...
def mostrar():
    ...
    with tab1:
        ...
        with st.container(border=True):
            ...
            f_fin = c2.date_input(
                "Fecha Término Global", value=date.today() + timedelta(days=30)
            )
        ...
        if dias_totales <= 0:
            ...
            df_visual["Inicio"] = df_visual["Inicio"].apply(
            ...
            if st.button("🚀 REGISTRAR PROYECTO NUEVO"):
                if not codigo or not nombre:
                    ...
                    st.error(
                        f"La suma de porcentajes debe ser 100% (Actual: {sum(pcts.values())}%)"
                    )
                else:
                    try:
                        datos_nube = {
    ...
    with tab2:
        ...
        bus = st.text_input(
            "🔍 Buscar...", placeholder="Escribe código, nombre o cliente"
        )
        df_p = obtener_proyectos(bus)
        if not df_p.empty:
            st.dataframe(
                df_p[["codigo", "proyecto_text", "cliente", "partida", "avance"]],
                hide_index=True,
            )
            st.divider()
            opciones_proy = df_p["proyecto_display"].tolist()
            seleccionado = st.selectbox(
                "🎯 Selecciona Proyecto para Eliminar:",
                ["-- Seleccionar --"] + opciones_proy,
            )
            if seleccionado != "-- Seleccionar --":
                ...
                with st.expander("🚫 Zona de Peligro"):
                    st.write(
                        "Esta acción eliminará el proyecto y TODOS sus registros asociados (Productos, Seguimientos e Incidencias)."
                    )
                    confirmar = st.checkbox(
                        f"Confirmo que deseo borrar permanentemente el proyecto {seleccionado}"
                    )
                    if st.button(
                        ...
                        if eliminar_proyecto_completo(id_sel):
                            ...
                            st.rerun()
                st.info(
                    "Ahora puedes ir a la pestaña **'📦 Matriz de Productos'** para cargar el Excel o agregar ítems manualmente."
                )
            st.divider()
            opciones_proy = df_p["proyecto_display"].tolist()
            seleccionado = st.selectbox(
                "🎯 Selecciona para gestionar Matriz de Productos:",
                ["-- Seleccionar --"] + opciones_proy,
            )
            if seleccionado != "-- Seleccionar --":
                id_sel = df_p[df_p["proyecto_display"] == seleccionado]["id"].values[0]
    ...
    with tab3:
        if st.session_state.get("id_p_sel"):
                        ...
                        if u and t:
                            try:
                                res_c = (
                                    ...
                                    .eq("proyecto_id", st.session_state.id_p_sel)
                                    .execute()
                                )
            ...
            with st.expander("📥 Importar Lista de Productos"):
                ...
                if f_up and st.button("🚀 Iniciar Importación Masiva"):
                    try:
                        ...
                        st.success(
                            f"✅ Se cargaron {len(lote)} productos nuevos con códigos correlativos."
                        )
                        st.rerun()
                    except Exception as e:
                        logging.exception("Unexpected error")
            ...
            res_p = (
                conectar()
                .table("productos")
                .select("codigo_etiqueta, ubicacion, tipo, ctd, ml")
                .eq("proyecto_id", st.session_state.id_p_sel)
            ...
            if res_p.data:
                ...
                if st.button("🗑️ Vaciar Matriz del Proyecto", type="primary"):
                    conectar().table("productos").delete().eq(
            ...
            st.info(
                "⚠️ Selecciona un proyecto en la pestaña 'Listado y Búsqueda' para gestionar su matriz."
            )