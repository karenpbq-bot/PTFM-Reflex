...
from base_datos import (
    conectar,
    obtener_proyectos,
    registrar_incidencia_detallada,
    obtener_incidencias_resumen,
)
...
def mostrar():
    ...
    with st.container(border=True):
        ...
        bus_proy = col_bus1.text_input(
            "🔍 Localizar Proyecto para el Requerimiento:",
        ...
        if not df_proyectos_all.empty:
            dict_proyectos = {
                row["proyecto_display"]: row["id"]
                for _, row in df_proyectos_all.iterrows()
            }
            proy_seleccionado_display = col_bus2.selectbox(
                "Confirmar Proyecto:", list(dict_proyectos.keys()), key="sel_proy_inc"
            )
            id_proyecto_actual = dict_proyectos[proy_seleccionado_display]
        else:
            st.warning("⚠️ No se encontró el proyecto. Escribe otro nombre.")
            return
    if "tmp_piezas" not in st.session_state:
        st.session_state.tmp_piezas = []
    if "tmp_mats" not in st.session_state:
    ...
    with tab_p:
        ...
        with st.expander("➕ Agregar Pieza a la Matriz", expanded=True):
            ...
            nveta = col_e.number_input(
                "No Veta (Dimención)", min_value=0, key="in_p_nveta"
            )
            mat = col_f.text_input("Material / Color", key="in_p_mat")
            rot = col_g.selectbox(
                "Rotación", [0, 1], help="0: No / 1: Si", key="in_p_rot"
            )
        ...
        if st.session_state.tmp_piezas:
            ...
            st.dataframe(
                pd.DataFrame(st.session_state.tmp_piezas), use_container_width=True
            )
            if st.button("🚀 ENVIAR REQUERIMIENTO (PIEZAS)", type="primary"):
                ...
                st.session_state.tmp_piezas = []
                st.success("Requerimiento enviado con éxito.")
                st.rerun()
    with tab_m:
        ...
        with st.container(border=True):
            ...
            if st.button("➕ Añadir Material"):
                st.session_state.tmp_mats.append(
                    {"descripcion": m_desc, "cantidad": m_cant, "observaciones": m_obs}
                )
                st.rerun()
        ...
        if not historial.empty:
            for _, inc in historial.iterrows():
                ...
                with st.expander(titulo_rotulo):
                    ...
                    v_alm = c1.checkbox(
                        "📦 Almacén", value=s1 == "🟩", key=f"c_alm_{id_i}"
                    )
                    if f_alm:
                        c1.caption(f"📅 {f_alm}")
                    v_sol = c2.checkbox(
                        "👤 Solicitante", value=s2 == "🟩", key=f"c_sol_{id_i}"
                    )
                    if f_sol:
                        c2.caption(f"📅 {f_sol}")
                    v_teo = c3.checkbox(
                        "🖥️ Teowin", value=s3 == "🟩", key=f"c_teo_{id_i}"
                    )
                    if f_teo:
                        c3.caption(f"📅 {f_teo}")
                    v_not = c4.text_input(
                        "Nota",
                        value=inc.get("obs_gestion", ""),
                        key=f"t_not_{id_i}",
                    ...
                    if c5.button("💾", key=f"b_sav_{id_i}"):
                        ...
                        p_datos = {
                            "fecha_almacen": f_hoy
                            if v_alm and (not (f_alm and str(f_alm).strip() != ""))
                            else f_alm
                            if v_alm
                            else "",
                            "fecha_solicitante": f_hoy
                            if v_sol and (not (f_sol and str(f_sol).strip() != ""))
                            else f_sol
                            if v_sol
                            else "",
                            "fecha_teowin": f_hoy
                            if v_teo and (not (f_teo and str(f_teo).strip() != ""))
                            else f_teo
                            if v_teo
                            else "",
                            "obs_gestion": v_not,
                    ...
                    st.markdown("---")
                    st.write(
                        f"**Motivo:** {inc['categoria']} | **Estado:** {inc['estado']}"
                    )
                    if inc.get("detalles"):
                        st.dataframe(
                            pd.DataFrame(inc["detalles"]), use_container_width=True
                        )
            st.divider()
        else:
