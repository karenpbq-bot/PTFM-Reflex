        ...
        with st.form("form_auto_cambio"):
            ...
            if st.form_submit_button("Actualizar mi contraseña"):
                res = (
                    ...
                    .select("contrasena")
                    .eq("nombre_usuario", st.session_state.usuario)
                    .execute()
                )
                if res.data and res.data[0]["contrasena"] == clave_act:
                    if nueva_cl == conf_cl and nueva_cl != "":
                        supabase.table("usuarios").update({"contrasena": nueva_cl}).eq(
    ...
    if rol_actual.lower() in roles_jefes:
        ...
        with tab1:
            with st.form("nuevo_usuario", clear_on_submit=True):
                ...
                u_rol = st.selectbox(
                    "Rol y Permisos", ["admin", "Gerente", "Supervisor"]
                )
                ...
                if "user_edit_id" in st.session_state:
                    with st.expander("📝 Editar Datos de Usuario", expanded=True):
                        with st.form("edit_form"):
                            n_nom = st.text_input(
                                "Nombre Real",
                                value=st.session_state.user_edit_data[
                                    "nombre_completo"
                                ],
                            )
                            n_usu = st.text_input(
                                "Usuario (Login)",
                                value=st.session_state.user_edit_data["nombre_usuario"],
                            )
                            n_rol = st.selectbox(
                                "Rol",
                                ["admin", "Gerente", "Supervisor"],
                                index=["admin", "Gerente", "Supevisor"].index(
                                    st.session_state.user_edit_data["rol"]
                                ),
                            )
